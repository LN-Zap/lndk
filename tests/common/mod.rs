...
impl LndNode {
    ...
    pub async fn get_info(&mut self) -> tonic_lnd::lnrpc::GetInfoResponse {
        let resp = if let Some(client) = self.client.clone() {
            ...
            let make_request = || async { client.clone().lightning().get_info(get_info_req.clone()).await };
    ...
    pub async fn connect_to_peer(
        ...
        let resp = if let Some(client) = self.client.clone() {
            let make_request = || async { client.clone().lightning().connect_peer(connect_req.clone()).await };
